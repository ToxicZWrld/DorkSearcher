...
HEADERS = {
    "User-Agent": random.choice([
        ...
        "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/109.0 Safari/537.36"
    ]),
...
def get_proxies_from_all_sources():
    ...
    proxies = set()
    try:
        geo = requests.get("https://proxylist.geonode.com/api/proxy-list?limit=300&page=1").json()
...
def dork_search(query, pages, proxy_pool, save=False, filename="results.txt"):
    ...
    for page in range(pages):
        proxy = local_proxy_pool[proxy_index % len(local_proxy_pool)]
        proxy_index += 1
        ...
        try:
            ...
            time.sleep(random.uniform(0.6, 1.2))  # delay aleatório por página
        except: continue
...
if __name__ == "__main__":
    ...
    print(f"{Azul}[*]{Reset} Iniciando buscas com discrição total...")
    with ThreadPoolExecutor(max_workers=min(7, len(dorks))) as executor:
        futures = [executor.submit(dork_search, dork, pages, valid_proxies, save, filename) for dork in dorks]
        for future in as_completed(futures):
            try:
                future.result()
            except Exception as e:
                print(f"{Vermelho}[!] Erro em uma dork: {e}{Reset}")
